...
To uninstall/delete the `nutanix-ccm` deployment:
...
The following table lists the configurable parameters of the Nutanix Cloud Provider chart and their default values.
...
```console
helm install nutanix-ccm oci://ghcr.io/nutanix-cloud-native/chart/nutanix-cloud-provider --version <version> -n kube-system --set prismCentralEndPoint=X.X.X.X --set username=admin --set password=xxxxxxxxx --set prismCentralInsecure=true
```
...
```console
helm install nutanix-ccm oci://ghcr.io/nutanix-cloud-native/chart/nutanix-cloud-provider --version <version> -n kube-system -f value.yaml
```
## Contributing
See the [contributing docs](https://github.com/nutanix-cloud-native/cloud-provider-nutanix/blob/main/CONTRIBUTING.md).
...
Issues and enhancement requests can be submitted in the [Issues tab of this repository](https://github.com/nutanix-cloud-native/cloud-provider-nutanix/issues). Please search for and review the existing open issues before submitting a new issue.
